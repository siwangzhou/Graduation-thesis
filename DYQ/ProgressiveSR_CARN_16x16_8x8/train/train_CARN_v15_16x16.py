...
from ProgressiveSR_CARN_16x16_8x8.ops.CARN_16x16 import LR_SR_x4_v15_quant
from ProgressiveSR_CARN_16x16_8x8.DataLoader_ImageSR.data_v14v15 import MyDataset, Test
...
if __name__ == '__main__':
    ...
    chpoint2 = torch.load("../experiment_CARN_16x16/v7/CS_CARN_P2P_x4_best.pt")
    v7_conv1_state_dict = {k.replace('conv1.', ''): v for k, v in chpoint2['model_state_dict'].items() if
                           k.startswith('conv1.')}
    v7_downsample_state_dict = {k.replace('layer1.', ''): v for k, v in chpoint2['model_state_dict'].items() if
                                k.startswith('layer1.')}
    net_G.conv1.load_state_dict(v7_conv1_state_dict)
    net_G.v7_downsample.load_state_dict(v7_downsample_state_dict)

    chpoint3 = torch.load("../experiment_CARN_16x16/v11/CS_CARN_P2P_x4_best.pt")
    v11_conv2_state_dict = {k.replace('conv2.', ''): v for k, v in chpoint3['model_state_dict'].items() if
                           k.startswith('conv2.')}
    v11_downsample_state_dict = {k.replace('layer1.', ''): v for k, v in chpoint3['model_state_dict'].items() if
                                k.startswith('layer1.')}
    net_G.conv2.load_state_dict(v11_conv2_state_dict)
    ...
    model_dir = "../experiment_CARN_16x16/v15"
    mkdirs(model_dir)
