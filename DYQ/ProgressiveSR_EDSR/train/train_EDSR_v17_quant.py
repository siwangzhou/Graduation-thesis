...
from ProgressiveSR_EDSR.ops.EDSR import LR_SR_x4_v17_quant
from ProgressiveSR_EDSR.DataLoader_ImageSR.data_v16v17 import MyDataset, Test
from ProgressiveSR_EDSR.utils import *
...
if __name__ == '__main__':
    ...
    net_G = LR_SR_x4_v17_quant(kwards=kwards).cuda()

    chpoint = torch.load("../experiment_EDSR/v2/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint2 = torch.load("../experiment_EDSR/v4/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint3 = torch.load("../experiment_EDSR/v7/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint4 = torch.load("../experiment_EDSR/v9/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint5 = torch.load("../experiment_EDSR/v11/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint6 = torch.load("../experiment_EDSR/v13/CS_EDSR_P2P_x4_best.pt")
    ...
    chpoint7 = torch.load("../experiment_EDSR/v15/CS_EDSR_P2P_x4_best.pt")
    ...
    for param in net_G.v11_downsample.parameters():
        param.requires_grad = False
    for param in net_G.conv5.parameters():
        param.requires_grad = False
    for param in net_G.v13_downsample.parameters():
        param.requires_grad = False
    for param in net_G.conv6.parameters():
        param.requires_grad = False
    for param in net_G.v15_downsample.parameters():
        param.requires_grad = False
    ...
    for epoch in range(start_epochs, epochs+1):
        ...
        if epoch % 1 == 0:
            ...
            if avg_y_psnr > best_psnr:
                ...
                torch.save(checkpoint_best, os.path.join(model_dir, 'CS_EDSR_P2P_x4_best.pt'))
        ...
        torch.save(checkpoint_latest, os.path.join(model_dir, 'CS_EDSR_P2P_x4_latest.pt'))
        ...
        fp = open(os.path.join(model_dir, 'CS_EDSR_P2P_x4.txt'), 'a+')
        fp.write(str_write)
        fp.close()
