...
if __name__ == '__main__':
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        output_dir = f"./results/hcd_v4_base_hcd_v2/Set5/"
        ...
        _, lr_v2 = load_image(f"./results/hcd_v2/Set5/{filename[0][:-4]}/best_x.png")
        # _, lr_v2 = load_image(f"../../ProgressiveSR_CARN/results/Set5/out_v2/lr/{filename[0]}")

        logger = setup_logging(os.path.join(output_dir, 'log.txt'))

        y = optimize_lr(logger, P2Pnet, hr_img, target_img, lr_v2, 15, 0.3, 20 / 255, path)
