...
def optimize_hr(logger, net, y, gt, Ny, epsilon, alpha, output_dir):
    ...
    delta_y_normalized = (best_delta_y - delta_y.min()) / (delta_y.max() - delta_y.min())
    # 设置阈值：小于阈值的部分置为 0
    delta_y_thresholded = delta_y_normalized.clone()
    # DIV2K 0.58
    delta_y_thresholded[delta_y_normalized < 0.55] = 0
    # 放大剩余部分以突出细节
    best_delta_y = torch.clamp(delta_y_thresholded * 2, 0, 1)
...
if __name__ == '__main__':
    ...
    output_dir = f"./results_delta_visual/hcd_v2/{data_name}/"
