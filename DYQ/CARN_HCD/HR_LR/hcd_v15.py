...
from ProgressiveSR_CARN.ops.CARN import LR_SR_x4_v15_quant
from ProgressiveSR_CARN.DataLoader_ImageSR.data_v14v15 import Test
...
psnrs_hr_lr_1 = AverageMeter()
ssims_hr_lr_1 = AverageMeter()
psnrs_hr_lr_2 = AverageMeter()
ssims_hr_lr_2 = AverageMeter()
base_psnrs = AverageMeter()
base_ssims = AverageMeter()


def optimize_hr(logger, net, y, gt, lr_v13, Ny, epsilon, alpha, output_dir):
    ...
    for i in range(Ny):
        ...
        LR_6 = net.conv6(lr_v13)

        LR_diff = net.layer1(y_prime)
        LR_diff = net.v15_quant(LR_diff)
    ...
    torch.save(best_y, output_dir + f"/best_y_padding32.pt")
    ...
    psnrs_hr_lr_1.update(best_pnsr, 1)
    ssims_hr_lr_1.update(best_ssim, 1)

    return gt


def optimize_lr(logger, net, y, gt, lr_v13, Nx, epsilon, alpha, output_dir):
    with torch.no_grad():
        LR_6 = net.conv6(lr_v13)
    ...
    for i in range(Nx):
        ...
        LR_cat = torch.cat((LR_6, net.v15_quant(LR_diff_prime)), dim=1)
        new_LR = rearrange(LR_cat, 'b (c a1 a2) h w -> b c (h a1) (w a2)', a1=14, a2=14, c=3)
    ...
    logger.info(f"[HR] Iteration\tbaseline_[psnr, ssim]: [{baseline[0]:.2f}, {baseline[1]:.4f}]\t"
                f"best_[psnr, ssim]: [{best_pnsr:.2f}, {best_ssim:.4f}]")
    psnrs_hr_lr_2.update(best_pnsr, 1)
    ssims_hr_lr_2.update(best_ssim, 1)
...
if __name__ == '__main__':
    kwards = {'scale': 2}
    P2Pnet = LR_SR_x4_v15_quant(kwards=kwards).cuda()
    chpoint = torch.load("../../ProgressiveSR_CARN/experiment_CARN/v15/CS_CARN_P2P_x4_best.pt")
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        lr_v13 = torch.load(f"./results/hcd_v13_2/Set5/{filename[0][:-4]}/best_x.pt")

        output_dir = f"./results/hcd_v15_1/Set5/"
        ...
        optimize_hr(logger, P2Pnet, hr_img, target_img, lr_v13,15,0.3, 20 / 255, path)
        logger.info(f"End test, base_result: [{base_psnrs.avg:.2f} / {base_ssims.avg:.4f}] \t best_result: [{psnrs_hr_lr_1.avg:.2f} / {ssims_hr_lr_1.avg:.4f}]\t")
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        output_dir = f"./results/hcd_v15_2/Set5/"
        path = os.path.join(output_dir, filename[0][:-4])
        os.makedirs(os.path.join(output_dir, filename[0][:-4]), exist_ok=True)
        logger = setup_logging(os.path.join(output_dir, 'log.txt'))
        hr_v15 = torch.load(f"./results/hcd_v15_1/Set5/{filename[0][:-4]}/best_y_padding32.pt")
        optimize_lr(logger, P2Pnet, hr_v15, target_img, lr_v13, 15, 0.3, 20 / 255, path)
        logger.info(f"End test, base_result: [{base_psnrs.avg:.2f} / {base_ssims.avg:.4f}] \t best_result: [{psnrs_hr_lr_2.avg:.2f} / {ssims_hr_lr_2.avg:.4f}]\t")