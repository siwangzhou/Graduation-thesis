...
def optimize_hr(logger, net, y, gt, lr_v11, Ny, epsilon, alpha, output_dir):
...
if __name__ == '__main__':
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        lr_v11 = torch.load(f"./results/hcd_v11_2/Set5/{filename[0][:-4]}/best_x.pt")

        output_dir = f"./results/hcd_v13_1/Set5/"
        ...
        optimize_hr(logger, P2Pnet, hr_img, target_img, lr_v11,15,0.3, 20 / 255, path)
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        output_dir = f"./results/hcd_v13_2/Set5/"
        path = os.path.join(output_dir, filename[0][:-4])
        os.makedirs(os.path.join(output_dir, filename[0][:-4]), exist_ok=True)
        logger = setup_logging(os.path.join(output_dir, 'log.txt'))
        hr_v13 = torch.load(f"./results/hcd_v13_1/Set5/{filename[0][:-4]}/best_y_padding32.pt")
        optimize_lr(logger, P2Pnet, hr_v13, target_img, lr_v11, 15, 0.3, 20 / 255, path)
