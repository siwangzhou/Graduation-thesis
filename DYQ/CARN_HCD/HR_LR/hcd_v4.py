...
from ProgressiveSR_CARN.ops.CARN import LR_SR_x4_v4_quant
from ProgressiveSR_CARN.DataLoader_ImageSR.data_v3v4 import Test
...
def optimize_hr(logger, net, y, gt, lr_v2, Ny, epsilon, alpha, output_dir):
    ...
    for i in range(Ny):
        ...
        LR_diff = net.v4_quant(LR_diff) #要或不要
...
def optimize_lr(logger, net, y, gt, lr_v2, Nx, epsilon, alpha, output_dir):
    with torch.no_grad():
        LR = net.conv1(lr_v2)
    ...
    for i in range(Nx):
        ...
        LR_cat = torch.cat((LR, net.v4_quant(LR_diff_prime)), dim=1)
        new_LR = rearrange(LR_cat, 'b (c a1 a2) h w -> b c (h a1) (w a2)', a1=9, a2=9, c=3)
...
if __name__ == '__main__':
    ...
    chpoint = torch.load("../../ProgressiveSR_CARN/experiment_CARN/v4/CS_CARN_P2P_x4_best.pt")
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        _, lr_v2 = load_image(f"./results/hcd_v2_1/Set5/{filename[0][:-4]}/best_x.png")

        output_dir = f"./results/hcd_v4_1/Set5/"
        ...
        optimize_hr(logger, P2Pnet, hr_img, target_img, lr_v2,15,0.3, 20 / 255, path)
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        output_dir = f"./results/hcd_v4_2/Set5/"
        path = os.path.join(output_dir, filename[0][:-4])
        os.makedirs(os.path.join(output_dir, filename[0][:-4]), exist_ok=True)
        logger = setup_logging(os.path.join(output_dir, 'log.txt'))
        hr_v4 = torch.load(f"./results/hcd_v4_1/Set5/{filename[0][:-4]}/best_y_padding32.pt")
        optimize_lr(logger, P2Pnet, hr_v4, target_img, lr_v2, 15, 0.3, 20 / 255, path)
