...
from ProgressiveSR_CARN.ops.CARN import LR_SR_x4_v17_quant
from ProgressiveSR_CARN.DataLoader_ImageSR.data_v16v17 import Test
...
def optimize_hr(logger, net, y, gt, lr_v15, Ny, epsilon, alpha, output_dir):
    ...
    for i in range(Ny):
        ...
        LR_7 = net.conv7(lr_v15)

        LR_diff = net.layer1(y_prime)
        LR_diff = net.v17_quant(LR_diff)
...
def optimize_lr(logger, net, y, gt, lr_v15, Nx, epsilon, alpha, output_dir):
    with torch.no_grad():
        LR_7 = net.conv7(lr_v15)
    ...
    for i in range(Nx):
        ...
        LR_cat = torch.cat((LR_7, net.v17_quant(LR_diff_prime)), dim=1)
        new_LR = rearrange(LR_cat, 'b (c a1 a2) h w -> b c (h a1) (w a2)', a1=15, a2=15, c=3)
...
if __name__ == '__main__':
    kwards = {'scale': 2}
    P2Pnet = LR_SR_x4_v17_quant(kwards=kwards).cuda()
    chpoint = torch.load("../../ProgressiveSR_CARN/experiment_CARN/v17/CS_CARN_P2P_x4_best.pt")
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        lr_v15 = torch.load(f"./results/hcd_v15_2/Set5/{filename[0][:-4]}/best_x.pt")

        output_dir = f"./results/hcd_v17_1/Set5/"
        ...
        optimize_hr(logger, P2Pnet, hr_img, target_img, lr_v15,15,0.3, 20 / 255, path)
    ...
    for test_batch, (_, hr_img, target_img, filename) in enumerate(test_data, start=0):
        ...
        output_dir = f"./results/hcd_v17_2/Set5/"
        path = os.path.join(output_dir, filename[0][:-4])
        os.makedirs(os.path.join(output_dir, filename[0][:-4]), exist_ok=True)
        logger = setup_logging(os.path.join(output_dir, 'log.txt'))
        hr_v17 = torch.load(f"./results/hcd_v17_1/Set5/{filename[0][:-4]}/best_y_padding32.pt")
        optimize_lr(logger, P2Pnet, hr_v17, target_img, lr_v15, 15, 0.3, 20 / 255, path)
